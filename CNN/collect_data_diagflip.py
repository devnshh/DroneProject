...
def collect_images():
    ...
    print(f"\nCollecting data for '{gesture_name}' [diagonal flip augmentation]")
    print(f"Saving to: {save_path}")
    print("Each capture saves 4 images: original + h-flip + v-flip + both")
    ...
    while True:
        ...
        cv2.imshow("Data Collection [CNN] (Diag-Flip)", display_frame)
        ...
        if key == ord('q'):
            ...
            hflipped = cv2.flip(frame, 1)
            hflip_name = f"{gesture_name}_{count}_hflip.jpg"
            hflip_path = os.path.join(save_path, hflip_name)
            cv2.imwrite(hflip_path, hflipped)

            vflipped = cv2.flip(frame, 0)
            vflip_name = f"{gesture_name}_{count}_vflip.jpg"
            vflip_path = os.path.join(save_path, vflip_name)
            cv2.imwrite(vflip_path, vflipped)

            bothflipped = cv2.flip(frame, -1)
            both_name = f"{gesture_name}_{count}_hvflip.jpg"
            both_path = os.path.join(save_path, both_name)
            cv2.imwrite(both_path, bothflipped)

            print(f"Saved: {img_name} + hflip + vflip + hvflip")
            count += 1
